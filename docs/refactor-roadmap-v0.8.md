...
安全网    bug修复   卫生清理   公共层     样式单源   拆mRibbon  悬浮窗     基建收尾   编码方案A
...
- 阶段 8（2026-07-26 用户拍板方案 A）已完成：.gitattributes 显式声明
  `.bas/.cls/.frm text eol=crlf` 与 `.frx/.dotm/.dotx/.bin/.png binary`，
  `Set-RAToolsAppVersion` 改按 GBK 写回（含中文回归测试 + 磁盘字节断言），
  契约文档 docs/vba-source-encoding.md。
