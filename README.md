...
Ribbon 的「宏列表」区域提供快捷悬浮窗入口。悬浮窗不会阻塞文档编辑，可拖动到合适位置，并会记住上次位置。界面复用 Ribbon 中相同功能的按钮图标，以紧凑、精确居中的分组多列图标块呈现，鼠标悬停可查看功能名称及操作说明。当前悬浮窗提供编号标题 1-4、无编号标题 1-4、格式刷、段落设置、段前分页、表标题、图标题、根据窗口自动调整表格、插入交叉引用、更新域，以及「超链接和域批量设置为蓝色」「接受修订并删除批注」「检测高亮内容」三个宏的快捷入口。
